...
\<img src="src/pdf\_reme/resources/images/pdf-reme-logo.jpeg" alt="PDF-REME Logo" width="260" />

\<br>\<br>

\<img src="src/pdf\_reme/resources/images/1.png" alt="PDF-REME Hero" width="100%" />
...
\<img src="src/pdf\_reme/resources/images/2.png" alt="PDF-REME Ana Sayfa Konsepti" width="100%" />
...
\<img src="src/pdf\_reme/resources/images/3.png" alt="PDF-REME Kütüphane Konsepti" width="100%" />
...
\- son kullanılan belgeler \`last\_opened\_at\` üzerinden sıralanabilir,
...
\<img src="src/pdf\_reme/resources/images/4.png" alt="PDF-REME PDF Düzenleyici Konsepti" width="100%" />
...
\<img src="src/pdf\_reme/resources/images/5.png" alt="PDF-REME Güvenli İçe Aktarma Akışı" width="100%" />
...
trashed\_from\_path = önceki kütüphane yolu

deleted\_at = silinme zamanı
...
\- önceki kütüphane konumunu \`trashed\_from\_path\` ile saklama,
...
\> \`cleanup\_expired()\` ile 30 günlük backend kontrolü hazırdır. Bu kontrolün uygulama başlangıç akışına otomatik bağlanması PySide6 uygulama shell'i / frontend entegrasyonu sırasında yapılacaktır.
...
\- \`generation\_type = "merge"\` ile üretilen belgenin işaretlenmesi.

Gerçek dosya testi:

\`\`\`bash

python scripts/manual\_merge\_test.py
...
python scripts/manual\_split\_test.py
...
\`reorder\_pages()\` verilen yeni sırayı birebir uygular.
...
\`swap\_pages()\` yalnızca belirtilen iki sayfanın yerini değiştirir ve mevcut \`reorder\_pages()\` altyapısını tekrar kullanır.
...
\`delete\_pages()\` seçilen sayfaları yeni PDF çıktısından kaldırır.
...
\`rotate\_pages()\` seçilen sayfaları 90 derecenin katlarıyla döndürür. Negatif açılar desteklenir ve çıktıdaki dönüş açısı normalize edilir.

**### Sayfa çoğaltma**

\`duplicate\_pages()\` seçilen her sayfanın bir kopyasını orijinal sayfanın hemen arkasına ekler.

**### Başka PDF'den sayfa ekleme**

\`insert\_pages()\` başka bir PDF'den seçilen sayfaları, seçim sırasını koruyarak belgenin başına veya belirtilen sayfanın arkasına ekler.

**### Boş sayfa ekleme**

\`insert\_blank\_page()\` referans sayfanın boyutlarını kullanarak belgenin başına veya belirtilen sayfanın arkasına boş bir sayfa ekler.
...
python scripts/manual\_page\_edit\_test.py

python scripts/manual\_page\_edit\_day11\_test.py
...
\<img src="src/pdf\_reme/resources/images/6.png" alt="PDF-REME Aktif Geliştirme" width="100%" />
...
python scripts/manual\_trash\_test.py
